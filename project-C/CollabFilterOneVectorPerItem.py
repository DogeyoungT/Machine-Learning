...
class CollabFilterOneVectorPerItem(AbstractBaseCollabFilterSGD):
    ...
    def predict(self, user_id_N, item_id_N,
        ...
        for index in range(N):
            i = user_id_N[index]
            j = item_id_N[index]
            yhat_N.append(mu + b_per_user[i] + c_per_item[j] + ag_np.dot(U[i],V[j]))
    ...
    def calc_loss_wrt_parameter_dict(self, param_dict, data_tuple):
        ...
        loss_total = ag_np.sum(ag_np.square(y_N - yhat_N)) + self.alpha*(ag_np.sum(ag_np.square(param_dict['U']))+ag_np.sum(ag_np.square(param_dict['V'])))
        return loss_total    
...
if __name__ == '__main__':
    ...
    model = CollabFilterOneVectorPerItem(
        n_epochs=30, batch_size=100, step_size=0.2,
        n_factors=50, alpha=1.0)
